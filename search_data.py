import os
import re
import time

from datetime import datetime
from bs4 import BeautifulSoup
from robot.libraries.BuiltIn import BuiltIn

timestamp = datetime.now().strftime("%Y%m%d")
screenshot_root = os.path.join(os.getcwd(), f"{timestamp}_screenshot")

def ensure_folder(path):
    if not os.path.exists(path):
        os.makedirs(path)

def search_data_from_html(page, html, target_item):
    soup = BeautifulSoup(html, "html.parser")
    step_list = soup.find("div", class_="step-list")
    get_mobile_name = re.search(r"mobile_name:\s*(\S+)", page.locator("#step-right").inner_text())
    if get_mobile_name:
        mobile_name = get_mobile_name.group(1)
        # print(f"mobile_name = {mobile_name}")

    for step in step_list:
        index = step.get("index")
        text = step.text.strip()
        # print(f"index: {index}, 內容: {text}")
        if index == '3' and '4[log]Ip Address' in text:
            screenshot(page, index, text, 'inet 10.0.0.', mobile_name, target_item)

        elif index == '4' and '5[log]SSID' in text:
            screenshot(page, index, text, 'SSID: ', mobile_name, target_item)
        
        elif index == '6' and '7[log]Ping Test' in text:
            screenshot(page, index, text, 'PING 8.8.8.8 (8.8.8.8)', mobile_name, target_item)

        elif index == '8' and '9[log]Ping Test' in text:
            screenshot(page, index, text, 'PING www.google.com', mobile_name, target_item)
        
        elif 'AdbShellError' in text:
            screenshot(page, index, text, 'PING 8.8.8.8 (8.8.8.8) or PING www.google.com', mobile_name, target_item)
        

            # print(f"-"*32)

def screenshot(page, index, text, keyword, mobile_name, target_item):
    page.locator('div.step[index="'+str(index)+'"]').click()
    ping_test_2 = page.locator("#step-right")
    if keyword in ping_test_2.inner_text():
        ensure_folder(screenshot_root)
        folder = re.match(r"^(#\s*\d+\[log\].*?)(\d+s\s+\d+ms)?$", text)
        if folder:
            folder_name = f"{folder.group(1)}"
        folder_path = os.path.join(screenshot_root, folder_name)
        ensure_folder(folder_path)
        page.screenshot(path=f"{folder_path}/{target_item}/{mobile_name}.png")
        print(f"處理 {mobile_name} index={index} {target_item} → {folder_name}")
    
        return True

    else:
        # print(f"❌ 關鍵字「{keyword}」未出現在 {text}！")
        BuiltIn().fail(f"❌ 關鍵字「{keyword}」未出現在 {text}！")


# if __name__ == "__main__":
#     extract_data_from_html()